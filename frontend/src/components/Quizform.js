...
const QuizCreationForm = (props) => {
  ...
  const initial=[];
  const [questions, setQuestions] = useState([{ questionText: '', options: [''], correctAnswerIndices:initial }]);
  ...
  const handleCorrectAnswerChange = (e, questionIndex, optionIndex) => {
    const updatedQuestions = [...questions];
    const selectedCorrectAnswers = updatedQuestions[questionIndex].correctAnswerIndices;
  ...
  const addQuestion = () => {
    setQuestions([...questions, { questionText: '', options: [''], correctAnswerIndices: [] }]);
  };
  ...
  return (
    <form onSubmit={handleSubmit}>
      <div>
        ...
        <input type="text" id="quizTitle" value={quizTitle} onChange={handleQuizTitleChange} minLength={3} required />
      </div>
      <div>
        <label htmlFor="quizDescription">Quiz Description:</label>
        <textarea id="quizDescription" value={quizDescription} onChange={handleQuizDescriptionChange} minLength={5} required />
      </div>
      ...
      {questions.map((question, questionIndex) => (
        <div key={questionIndex}>
          ...
          {question.options.map((option, optionIndex) => (
            <div key={optionIndex}>
              ...
              <input
                ...
                onChange={(e) => handleCorrectAnswerChange(e, questionIndex, optionIndex)}
              />
              <label htmlFor={`correctAnswer-${questionIndex}-${optionIndex}`}>Correct Answer</label>
              {optionIndex > 0 && (
          ...
          <div>
          <button type="button " id='add_button' onClick={() => addOption(questionIndex)}>
              Add Option
            </button>
            <button type="button " id= 'remove_button' onClick={() => removeQuestion(questionIndex)}>
              Remove Question
            </button>
          </div>
        </div>
